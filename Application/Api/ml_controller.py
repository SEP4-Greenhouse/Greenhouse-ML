...
async def predict(payload: PredictionRequestDto, request: Request):
    """
    Predict hours until next watering is needed.
    
    This endpoint takes sensor readings and plant information and returns
    a prediction of how many hours until the next watering is required.
    
    Parameters:
        payload (PredictionRequestDto): Sensor readings and plant information
        
    Returns:
        PredictionResultDto: Contains the timestamp, predicted hours until watering, and model version
    """
    try:
        ...
        print(f"[ML_API] Successful prediction: {result.predictedHoursUntilWatering:.2f} hours using model {result.modelVersion}")
        return result
